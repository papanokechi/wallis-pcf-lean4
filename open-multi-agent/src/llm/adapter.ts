...
export async function createAdapter(
  ...
  switch (provider) {
    ...
    case 'copilot': {
      ...
      // Reuse a single CopilotAdapter so all agents share one auth session.
      if (!apiKey && _cachedCopilotAdapter) return _cachedCopilotAdapter
      const { CopilotAdapter } = await import('./copilot.js')
      const adapter = new CopilotAdapter(apiKey)
      if (!apiKey) _cachedCopilotAdapter = adapter
      return adapter
    }
    case 'gemini': {
...
/** Cached singleton for the Copilot adapter to avoid repeated device flows. */
let _cachedCopilotAdapter: LLMAdapter | null = null
