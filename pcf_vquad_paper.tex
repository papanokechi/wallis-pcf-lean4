...
\paragraph{Modular input for Lemma~K.}
The Kloosterman-type sum $A_k(1,q) = \sum_{d \bmod q,\, \gcd(d,q)=1}
\exp(-2\pi i k\,\theta(d,q))\,e^{2\pi i d^{-1}/q}$,
where $\theta(d,q) = (a+d)/(12q) - s(d,q)$ and $a \equiv d^{-1} \pmod{q}$,
is attached to the $\eta(\tau)^{-k}$ multiplier via
Apostol~\cite{apostol1976} Theorem~3.4 (see also Iwaniec--Kowalski~\cite{IK2004}
\S2.8); the sign $\exp(-2\pi i k\,\theta)$ is the correct inverse character.
Since the eta multiplier has unit modulus, Weil's bound for classical
Kloosterman sums gives $|A_k(1,q)|\ll d(q)q^{1/2}$ uniformly in $k$. In our
explicit sweep for $1\le k\le 24$ and $q\le 1000$ we find
\[
  C_k:=\max_{q\le 1000} \frac{|A_k(1,q)|}{d(q)q^{1/2}} \le 1.4368,
\]
and the values are exactly $12$-periodic in $k$, consistent with the
underlying $24$th-root eta multiplier.
