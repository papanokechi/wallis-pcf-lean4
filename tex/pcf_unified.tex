...
\title{Polynomial Continued Fractions: Proved Families,\\
Casoratian Series Identities, and 482 New Irrational Constants}
\author{Papanokechi}
\date{April 2026}
...
(PCF) identities sharing the common denominator~$\beta(n) = 3n{+}1$.
The \emph{Logarithmic Ladder},
$\PCF(-kn^2,\,(k{+}1)n{+}k) = 1/\ln(k/(k{-}1))$
for all real $k > 1$,
is \textbf{proved} by exhibiting closed-form convergents that reduce to
the Taylor series~$-\ln(1{-}x)/x$.
The \emph{Pi Family},
$\PCF(-n(2n{-}(2m{+}1)),\,3n{+}1) = 2^{2m+1}/(\pi\binom{2m}{m})$,
is \textbf{proved} at the base case $m=0$ ($2/\pi$) via the arcsin series,
and at $m=1$ ($4/\pi$) via a novel Casoratian (discrete Wronskian) argument
that yields a new rapidly convergent series identity
\[
  \frac{\pi}{4} = 1 - \sum_{k=1}^{\infty}
  \frac{k!}{\dff{2k{-}1}\,(k^2{+}3k{+}1)(k^2{+}k{-}1)},
\]
with exponential convergence rate~$O(2^{-N}/N^{7/2})$.

Our main result unifies both integer and half-integer specialisations into
a single continuous identity: for all real $m > -\frac{1}{2}$, the PCF
converges to $2\,\Gamma(m{+}1)/(\sqrt{\pi}\,\Gamma(m{+}\half))$.
This is established via a raising operator mapping solutions between
adjacent recurrences, combined with Poincar\'e growth analysis.
...
We prove that the $m=1$ CF does not arise from any ${}_2F_1$ via the Gauss
continued-fraction theorem; the associated series lies in the
class~${}_2F_3$ over~$\mathbb{Q}(\sqrt{5})$.
A comparison with Brouncker's classical CF for~$4/\pi$ reveals dramatic
structural differences: exponential convergence~$O(2^{-n})$
versus~$O(1/n)$, with a factor of~$10^4$ at~$n=8$.

Finally, we catalogue 482 provably irrational constants from
quadratic-denominator generalised continued fractions, all certified
by ball arithmetic to $1\,500{+}$ digits, with systematic PSLQ searches
against 15 standard constants finding no algebraic relations.
\end{abstract}

\noindent
\textbf{2020 Mathematics Subject Classification:}
11A55 (primary), 11J70, 11Y60, 33B15, 40A15.

\noindent
\textbf{Keywords:}
polynomial continued fractions, Gamma function, irrationality,
Casoratian, series identity, PSLQ, Wallis products.
...
celebrated proof of irrationality of~$\zeta(3)$~\cite{apery1979} hinges
on a PCF with degree-six numerator.
The Ramanujan Machine project~\cite{raayoni2021,elimelech2024} has revived
systematic search for PCF representations of mathematical constants.

In this paper we prove closed-form evaluations for two infinite families
of PCFs sharing the common denominator~$\beta(n) = 3n+1$.
The first, the \emph{Logarithmic Ladder} (\S\ref{sec:log}), produces
$1/\ln(k/(k{-}1))$ for all real~$k > 1$.
The second, the \emph{Pi Family} (\S\S\ref{sec:pi}--\ref{sec:gamma}),
converges to~$2\,\Gamma(m{+}1)/(\sqrt{\pi}\,\Gamma(m{+}\half))$
for all real~$m > -\frac{1}{2}$, unifying $\pi$-formulas at integer~$m$
with exact Wallis rationals at half-integer~$m$ into a single continuous
Gamma-function identity (Theorem~\ref{thm:gamma}).
For the $m=1$ specialisation~$S = 4/\pi$, we give a self-contained proof
via the discrete Wronskian (Casoratian), yielding a new series for~$\pi/4$
(\S\ref{sec:casoratian}).
We establish that this CF does not arise from any~${}_2F_1$ via the Gauss
theorem (\S\ref{sec:gauss}), compare it with Brouncker's classical CF
(\S\ref{sec:brouncker}), and present a catalogue of 482 new irrational
constants from quadratic GCFs (\S\ref{sec:vquad}).

We emphasise the asymmetry of our results: the Logarithmic Ladder and the
base cases $m = 0, 1$ of the Pi Family are fully proved; the Gamma identity
for general~$m$ is established conditionally on a ratio formula
(Conjecture~\ref{conj:ratio}) supported by $500{+}$ digits of evidence.
...
$[1.5, 100]$ to $90$--$120$ matching digits.
\end{remark}
...
\begin{remark}
The recurrence is verified numerically to $500{+}$ matching digits
for $m = 0, 1, \ldots, 10$ (and at half-integer values $m = 0.5, \ldots, 10.5$).
...
This is verified for $m = 0, \ldots, 12$ and $n \le 20$.
...
Conjecture~\ref{conj:ratio} (supported by $500{+}$ digit evidence).
...
\begin{remark}
This connects our PCF to the classical identity
$\pi/2 = \sum_{k=0}^{\infty} k!/\dff{2k{+}1}$
...
as special cases.
Two additional sub-families were discovered and verified to
80~digits:
\begin{itemize}
...
\item \textbf{Quadratic GCFs.}
  ...
  has any closed form.
  This constant resists $3\,400{+}$ PSLQ tests against
