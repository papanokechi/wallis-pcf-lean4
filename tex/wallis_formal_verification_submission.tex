...
A direct formalization of the continued fraction itself is not the most transparent Lean entry point: the convergent bookkeeping hides the only genuinely rigid part of the argument.  We therefore separate the proof into an algebraic transport step and an analytic base case.  First, a factorial normalization rewrites the recurrence in a form on which the parameter shift $m \mapsto m+1$ becomes linear.  Second, Mathlib's Wallis-product infrastructure \cite{mathlib2020,borwein1987pi} supplies the analytic anchor at $m=0$, after which a single constant-multiple \texttt{Tendsto} argument propagates the closed form through the entire family.

The heart of the argument is the intertwining lemma.  Once Lean confirms that the affine operator $L_m$ transports any solution of the normalized $m$-recurrence to a solution of the normalized $(m+1)$-recurrence, the whole one-parameter tower reduces to one base case and the elementary ratio
\[
  \frac{\WallisValue{m+1}}{\WallisValue{m}} = \frac{2(m+1)}{2m+1}.
\]
This is why the formal proof is structured around the intertwiner rather than around ad hoc computations of individual convergents.
...
\end{equation}
Symbolic elimination first suggested \eqref{eq:intertwiner}; Lean~4 then verified that it is the correct operator linking adjacent parameters.  For audit purposes, the current file exposes the certified chain
\begin{lstlisting}
...
\end{lstlisting}
and these checks elaborate without error in the zero-\texttt{sorry} development.
...
Formalized analysis and number theory in proof assistants have advanced rapidly, but explicit formal verifications of continued-fraction evaluations remain rare.  On the Lean side, the relevant background comes from the growing \texttt{mathlib} ecosystem \cite{mathlib2020}; in Coq, the Coquelicot library provides a mature real-analysis framework for machine-checked transcendental arguments \cite{boldo2015coquelicot}; and Isabelle/HOL has produced substantial formalizations of Stirling's formula, the prime number theorem, and the irrationality of $\zeta(3)$ \cite{eberl2016stirling,eberl2018pnt,eberl2019zeta3}.  The present Wallis family fits naturally into that line of work, but is unusual in treating a concrete parametric continued-fraction identity rather than an isolated asymptotic or arithmetic theorem.

Within the continued-fraction literature, Wallis-type product identities and their generalized continued-fraction avatars go back to the classical sources of Wall, Lorentzen--Waadeland, and Borwein--Borwein \cite{wall1948analytic,lorentzen1992continued,borwein1987pi}.  Our search-driven viewpoint is complementary to that tradition: instead of beginning with a known special function and deriving a continued fraction, we begin with a low-height PCF sweep and recover the special-function meaning afterward.  The paper also connects to the authors' broader experimental program on proved PCF families for logarithmic and $4/\pi$ constants \cite{papanokechi2026pcf}.

The Ramanujan Machine project \cite{raayoni2021} provides the closest methodological antecedent.  Like that program, we use automated search to surface plausible identities among fundamental constants; unlike a purely conjectural pipeline, however, the present work closes the loop with a full Lean~4 verification.  In that sense the manuscript is a case study in how symbolic search, high-precision numerics, and proof assistants can be integrated into a single research workflow.
...
\begin{enumerate}[label=\textup{(\arabic*)}]
  ...
  \item To extend the present argument to the full degree-$d$ Dichotomy conjecture, one would need a symbolic procedure that produces the higher-order analogue of the intertwining operator together with an analytic base case identifying the correct limiting constant.  The current Lean file suggests that, once those two ingredients are available, the induction and \texttt{Tendsto} transport can be reused with only modest changes.
\end{enumerate}
