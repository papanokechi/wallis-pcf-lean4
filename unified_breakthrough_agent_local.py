...
FRONTIER_STATE_FILE = STATE_DIR / "frontier_state.json"
...
class SemanticMemory:
    ...
    def reset_gap_history(self, gap_id: str) -> int:
        """Remove accumulated failure signatures for one frontier gap."""
        before = len(self.failure_signatures)
        self.failure_signatures = [
            f for f in self.failure_signatures if f.get("gap_id") != gap_id
        ]
        return before - len(self.failure_signatures)
...
class KnowledgeGraph:
    ...
    def _load_frontier_state(self):
        if not FRONTIER_STATE_FILE.exists():
            return
        try:
            data = json.loads(FRONTIER_STATE_FILE.read_text(encoding="utf-8"))
            persisted = {gid for gid in data.get("resolved_gaps", []) if gid}
            if persisted:
                self.resolved_gaps.update(persisted)
                info(f"Loaded {len(persisted)} resolved frontier gaps from state")
        except Exception as e:
            warn(f"frontier_state.json load failed: {e}")
    ...
    def save_state(self):
        ...
        payload = {
            "version": VERSION,
            "saved_at": datetime.now().isoformat(),
            "resolved_gaps": sorted(gid for gid in self.resolved_gaps if gid),
        }
        FRONTIER_STATE_FILE.write_text(json.dumps(payload, indent=2), encoding="utf-8")

    def get_gap(self, gap_id: str) -> Optional[FrontierGap]:
        for gap in self.gaps:
            if gap.id == gap_id:
                return gap
        return None

    def known_gap_ids(self) -> list[str]:
        return [gap.id for gap in self.gaps]

    def mark_resolved(self, gap_id: str) -> bool:
        if self.get_gap(gap_id) is None:
            return False
        self.resolved_gaps.add(gap_id)
        self.save_state()
        self.rank_gaps()
        return True

    def unmark_resolved(self, gap_id: str) -> bool:
        if self.get_gap(gap_id) is None:
            return False
        self.resolved_gaps.discard(gap_id)
        self.save_state()
        self.rank_gaps()
        return True
    ...
    def rank_gaps(self) -> list[FrontierGap]:
        ...
        self.gaps.sort(key=lambda g: (g.id in self.resolved_gaps, -g.score))
        return self.gaps
...
# ══════════════════════════════════════════════════════════════════════════
# CHATBRIDGE — API-free local relay via VS Code stdin/stdout
# ══════════════════════════════════════════════════════════════════════════

class ChatBridge:
    """
    Relay that talks to an LLM without an API key by reading/writing the
    VS Code terminal.

    How it works:
      1. The agent writes a JSON "request" packet to BRIDGE_REQUEST_FILE.
      2. It prints a human-readable prompt to stdout and waits for the user
         to paste the LLM's reply into the terminal (or into BRIDGE_REPLY_FILE).
      3. The reply is returned as a string to the caller.

    Two input modes (auto-detected at runtime):
      INTERACTIVE  — user pastes reply at the >>> prompt (one or more lines,
                     terminated by a line containing only "END" or "###").
      FILE         — user saves reply to BRIDGE_REPLY_FILE before pressing Enter.

    AUTO_REPLY mode (set BRIDGE_AUTO_REPLY=1 env var):
      Returns a minimal valid JSON stub — useful for dry-run / CI testing
      without any human input at all.

    The bridge is fully transparent:  every request is also written to
    BRIDGE_LOG_FILE so you can review the conversation history later.
    """

    REQUEST_FILE = WORKSPACE / "bridge_request.json"
    REPLY_FILE   = WORKSPACE / "bridge_reply.txt"
    LOG_FILE     = WORKSPACE / "bridge_log.jsonl"
    TERMINATOR   = {"END", "###", "---END---"}

    # Auto-reply stubs (used when BRIDGE_AUTO_REPLY=1)
    _AUTO_PRODUCER = json.dumps({"hypotheses": [
        {"claim": "[AUTO] Conservative stub hypothesis",
         "mechanism": "Placeholder mechanism",
         "testable_prediction": "No numeric prediction",
         "failure_condition": "Any contradiction",
         "domain": "mathematics",
         "domains_touched": ["mathematics"],
         "plausibility": 0.5,
         "surprise_if_true": 0.5},
        {"claim": "[AUTO] Balanced stub hypothesis",
         "mechanism": "Placeholder mechanism",
         "testable_prediction": "No numeric prediction",
         "failure_condition": "Any contradiction",
         "domain": "mathematics",
         ...
         "plausibility": 0.5,
         "surprise_if_true": 0.5},
        {"claim": "[AUTO] Wild stub hypothesis",
         "mechanism": "Speculative bridge mechanism",
         "testable_prediction": "Speculative prediction",
         "failure_condition": "Any contradiction",
         "domain": "physics",
         "domains_touched": ["physics", "mathematics"],
         "plausibility": 0.4,
         "surprise_if_true": 0.8},
    ]})

    _AUTO_ADVERSARY = json.dumps({"verdicts": [
        {"hypothesis_index": i,
         "scores": {"N": 0.55, "F": 0.57, "E": 0.40, "C": 0.42},
         "score_justifications": {
             "N": "Auto stub", "F": "Auto stub",
             "E": "Auto stub", "C": "Auto stub"},
         "logical_attack": "Auto stub — not a real evaluation",
         "boundary_case": "N/A",
         "hidden_assumption": "N/A",
         "survivability_score": 0.5,
         "resurrectability": 0.3}
        for i in range(3)
    ]})
    ...
    @classmethod
    def _auto_reply(cls, label: str) -> str:
        """Return a canned stub reply without any user interaction."""
        lbl = label.lower()
        if "adversar" in lbl or "critic" in lbl:
            stub = cls._AUTO_ADVERSARY
        else:
            stub = cls._AUTO_PRODUCER
        warn(f"[AUTO_REPLY] Returning stub for: {label}")
        return stub

    @classmethod
    def query(cls, system_prompt: str, user_prompt: str,
              label: str = "", max_tokens: int = 2048,
              temperature: float = 0.8) -> str:
        """
        Main entry point.  Blocks until the user provides a reply.
        Returns the reply text (stripped).
        """
        cls._call_count += 1
        call_id = f"call_{cls._call_count:04d}"

        # ── Auto-reply mode (BRIDGE_AUTO_REPLY=1 env var or --dry-run) ─
        if os.environ.get("BRIDGE_AUTO_REPLY", "").strip() == "1":
            return cls._auto_reply(label or call_id)

        # ── 1. Build the full prompt shown to the LLM ──────────────────
        full_prompt = (
            f"SYSTEM:\n{system_prompt}\n\n"
            f"USER:\n{user_prompt}"
        )

        # ── 2. Write request packet ─────────────────────────────────────
        request_time = time.time()          # timestamp BEFORE writing request
        packet = {
            "call_id":     call_id,
            "label":       label or f"Phase call #{cls._call_count}",
            "timestamp":   datetime.now().isoformat(),
            "temperature": temperature,
            "max_tokens":  max_tokens,
            "prompt":      full_prompt,
        }
        cls.REQUEST_FILE.write_text(
            json.dumps(packet, ensure_ascii=False, indent=2), encoding="utf-8")

        # Append to log
        cls.LOG_FILE.parent.mkdir(parents=True, exist_ok=True)
        with open(cls.LOG_FILE, "a", encoding="utf-8") as lf:
            lf.write(json.dumps({"role": "request", **packet}) + "\n")

        # ── 3. Print the prompt for the user ────────────────────────────
        sep = "=" * 72
        print(f"\n{C.PURPLE}{sep}")
        print(f"  [!] CHATBRIDGE — {label or call_id}  (T={temperature})")
        print(f"  Copy the block below into Claude / ChatGPT / any LLM")
        print(f"  Then paste the reply here (or save to bridge_reply.txt)")
        print(f"{sep}{C.RESET}")
        print(full_prompt)
        print(f"\n{C.CYAN}{sep}")
        print(f"  Options:")
        print(f"    A) Paste reply here line-by-line, finish with: END")
        print(f"    B) Save reply to:  {cls.REPLY_FILE}")
        print(f"       then press Enter here.")
        print(f"{sep}{C.RESET}")

        # ── 4. Collect the reply ────────────────────────────────────────
        # NOTE: stale-file clear happens INSIDE _collect_reply, AFTER the
        # prompt is printed — this prevents the race condition where an
        # auto-responder writes reply.txt in response to request.json
        # before the agent reaches the poll loop.
        reply = cls._collect_reply(request_time)

        # Append reply to log
        with open(cls.LOG_FILE, "a", encoding="utf-8") as lf:
            lf.write(json.dumps({
                "role": "reply", "call_id": call_id,
                "timestamp": datetime.now().isoformat(),
                "text": reply,
            }) + "\n")

        ok(f"ChatBridge reply received ({len(reply)} chars)")
        return reply

    @classmethod
    def _collect_reply(cls, request_time: float = 0.0) -> str:
        """
        Collect reply via file-poll or interactive input.

        request_time: time.time() when bridge_request.json was written.
        Any bridge_reply.txt with mtime < request_time is stale and cleared.
        Files written AFTER request_time are valid (auto-responder output).
        This eliminates the race: agent no longer clears a reply the
        auto-responder already wrote in response to seeing request.json.
        """
        # ── Stale-file clear (timestamp-guarded, AFTER prompt is printed) ─
        if cls.REPLY_FILE.exists():
            try:
                if cls.REPLY_FILE.stat().st_mtime < request_time:
                    cls.REPLY_FILE.write_text("", encoding="utf-8")
                # else: written after request — may already be the reply
            except Exception:
                pass

        # ── Force file-poll mode via env var ────────────────────────────
        if os.environ.get("CHATBRIDGE_FILE_MODE", "").strip() == "1":
            return cls._poll_file(timeout=600)

        # ── Non-interactive stdin ────────────────────────────────────────
        if not sys.stdin.isatty():
            return cls._poll_file(timeout=600)

        # ── Interactive: quick-check file first, then line collection ───
        # Give auto-responder ~0.5s to react to request.json
        time.sleep(0.5)
        quick = cls._try_file()
        if quick:
            return quick
        ...
        while True:
            ...
            if line.strip() in cls.TERMINATOR:
                break
    ...
    @classmethod
    def _try_file(cls) -> str:
        """Read bridge_reply.txt if it has content."""
        try:
            if cls.REPLY_FILE.exists():
                text = cls.REPLY_FILE.read_text(encoding="utf-8").strip()
                if text:
                    cls.REPLY_FILE.write_text("", encoding="utf-8")
                    return text
        except Exception:
    ...
    @classmethod
    def _poll_file(cls, timeout: int = 600) -> str:
        """Poll bridge_reply.txt until content appears (non-interactive mode)."""
        ...
        while time.time() < deadline:
            ...
            time.sleep(2)
        raise TimeoutError(
            f"ChatBridge: no reply received within {timeout}s.\n"
            f"  Save LLM output to: {cls.REPLY_FILE}")


# ══════════════════════════════════════════════════════════════════════════
# API LAYER  (multi-backend: Anthropic SDK > OpenAI SDK > ChatBridge)
# ══════════════════════════════════════════════════════════════════════════

# Label map: maps (system_prompt snippet → phase name) for readable output
def _infer_label(system_prompt: str) -> str:
    triggers = [
        ("Failure Archaeologist",   "Archaeology"),
        ("adversary",               "Adversarial critic"),
        ("producer",                "Hypothesis producer"),
        ("synthesizer",             "Synthesis"),
        ("Frontier",                "Frontier scan"),
        ("verifier",                "Verifier"),
...
def call_api(system_prompt: str, user_prompt: str, *,
    ...
    Priority order:
      ...
      2. OpenAI SDK     (if OPENAI_API_KEY is set and non-Anthropic base)
      3. urllib direct  (if key present, either backend)
      4. ChatBridge     (API-free local relay — always available as fallback)
    """
    label = _infer_label(system_prompt)
    ...
    if is_anthropic and HAS_ANTHROPIC and api_key:
        try:
            ...
            warn(f"Anthropic SDK failed ({e}), falling back to ChatBridge")
    ...
    if not is_anthropic and HAS_OPENAI and api_key:
        try:
            ...
            warn(f"OpenAI SDK failed ({e}), falling back to ChatBridge")

    # ── Tier 3: urllib direct ───────────────────────────────────────────
    if api_key:
        try:
            if is_anthropic:
            ...
            return data["content"][0]["text"] if is_anthropic \
                else data["choices"][0]["message"]["content"]
        except Exception as e:
            warn(f"Direct HTTP failed ({e}), falling back to ChatBridge")

    # ── Tier 4: ChatBridge (no API key required) ────────────────────────
    return ChatBridge.query(
...
"""
    ...
    # ── Bridge mode: single combined prompt (1 paste round instead of 3) ──
    using_bridge = not api_key  # ChatBridge is used when no key is present
    if using_bridge:
        combined_prompt = base_prompt.replace("{count}", "3")
        combined_prompt += (
            "\n\nGenerate exactly 3 hypotheses total — one for each thinking style:\n"
            "  [0] CONSERVATIVE (T=0.3): grounded, incremental, builds on known results\n"
            "  [1] BALANCED     (T=0.8): moderate novelty, plausible mechanism\n"
            "  [2] WILD         (T=1.3): speculative, cross-domain, challenges assumptions\n"
            "\nReturn a JSON object with a 'hypotheses' array of exactly 3 items.\n"
            "Each item must have: claim, mechanism, testable_prediction, failure_condition,\n"
            "domain, domains_touched (list), plausibility (0-1), surprise_if_true (0-1).\n"
        )
        tier_names = ["conservative", "balanced", "wild"]
        info("  [bridge] Single combined prompt (3 hypotheses)...")
        try:
            raw = call_api(genome.producer_prompt, combined_prompt,
                           api_base=api_base, api_key=api_key, model=model,
                           max_tokens=2500, temperature=0.8)
            data = parse_json_response(raw)
            for i, h_data in enumerate(data.get("hypotheses", [])):
                tier_name = tier_names[i] if i < len(tier_names) else "balanced"
                h = Hypothesis(
            ...
            err(f"  [bridge] Combined prompt error: {e}")

    else:
        # ── API mode: separate calls per tier (original behaviour) ─────────
...
class UnifiedBreakthroughAgent:
    ...
    def phase_summarize(self, gap: FrontierGap, hypotheses: list[Hypothesis],
        ...
        if verdict != "breakthrough":
            lead = max(survivors, key=lambda h: h.b_score(), default=None)
            self.memory.add_failure(
                gap_id=gap.id,
                hypothesis=lead.claim if lead else gap.description,
                reason=f"Cycle verdict={verdict}; B={best_b:.3f}; survivors={len(survivors)}",
                method="cycle",
                signature={
                    "verdict": verdict,
                    "b_score": round(best_b, 6),
    ...
    def run_cycle(self) -> Optional[IterationRecord]:
        ...
        if not self.api_key:
            warn("No API key set — running in ChatBridge mode (interactive relay).")
...
Examples:
  ...
  python unified_breakthrough_agent.py --mode "Physics/Math" --target "Scaling Laws"
    ...
    parser.add_argument("--mark-resolved", metavar="GAP_ID",
                        help="Mark a frontier gap as resolved and persist the change")
    parser.add_argument("--unmark-resolved", metavar="GAP_ID",
                        help="Remove the resolved mark from a frontier gap")
    parser.add_argument("--reset-gap", metavar="GAP_ID",
                        help="Clear one gap's accumulated failure history so its score starts fresh")
    parser.add_argument("--dry-run", action="store_true",
                        help="Run 1 cycle with auto-stub replies (no LLM, no pasting) — tests plumbing")
    parser.add_argument("--bridge-test", action="store_true",
                        help="Send one test prompt through ChatBridge and exit")
    parser.add_argument("--no-bridge", action="store_true",
                        help="Abort if no API key (disable ChatBridge fallback)")
    ...
    if args.dry_run:
        os.environ["BRIDGE_AUTO_REPLY"] = "1"
        warn("DRY-RUN mode: all LLM calls return stubs (no pasting required)")
        args.cycles = 1

    if args.no_bridge and not args.api_key:
        err("No API key and --no-bridge set. Aborting.")
        sys.exit(1)

    if args.bridge_test:
        print("\nChatBridge smoke test — paste any text and end with END")
        result = ChatBridge.query(
            system_prompt="You are a helpful assistant.",
            user_prompt="Say hello and confirm ChatBridge is working.",
            label="Bridge smoke test",
        )
        ok(f"Bridge test reply received:\n{result[:300]}")
        return
    ...
    if args.mark_resolved:
        ...
        if agent.knowledge.mark_resolved(args.mark_resolved):
            ok(f"Marked gap resolved: {args.mark_resolved}")
        else:
            err(f"Unknown gap: {args.mark_resolved}")
            info(f"Known gaps: {', '.join(agent.knowledge.known_gap_ids())}")
            sys.exit(1)

    if args.unmark_resolved:
        management_action = True
        if agent.knowledge.unmark_resolved(args.unmark_resolved):
            ok(f"Re-enabled gap: {args.unmark_resolved}")
        else:
            err(f"Unknown gap: {args.unmark_resolved}")
            info(f"Known gaps: {', '.join(agent.knowledge.known_gap_ids())}")
            sys.exit(1)

    if args.reset_gap:
        management_action = True
        if agent.knowledge.get_gap(args.reset_gap) is None:
            err(f"Unknown gap: {args.reset_gap}")
            info(f"Known gaps: {', '.join(agent.knowledge.known_gap_ids())}")
            sys.exit(1)
        removed = agent.memory.reset_gap_history(args.reset_gap)
        agent.memory.save()
        agent.knowledge.rank_gaps()
        ok(f"Reset failure history for {args.reset_gap} ({removed} signature(s) removed)")

    if management_action and not any([
        args.status,
        args.frontier,
        args.history,
        args.mutate,
