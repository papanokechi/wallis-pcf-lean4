...
\author{Autonomous Discovery Relay Chain --- Rounds 2--10\\
\small Multi-Agent Conjecture Discovery Framework --- Yokohama, Japan}
...
Let $\nu_{\eta}$ denote the Dedekind eta multiplier on $\mathrm{SL}_2(\mathbb Z)$, so $\nu_{\eta}^{24}=1$. For fixed $k\ge1$, the Rademacher coefficients for $\eta(\tau)^{-k}$ are
\[
  A_k(n,c)
  = \sum_{\substack{0\le d<c\\ (d,c)=1}}
    \nu_{\eta}(\gamma_{c,d})^{-k}
    e\!\left(\frac{(n-k/24)d}{c}-\frac{k a}{24c}\right),
  \qquad
  \gamma_{c,d}=\begin{pmatrix} a & *\\ c & d\end{pmatrix}\in \mathrm{SL}_2(\mathbb Z),
\]
where $ad\equiv 1\pmod c$ and $e(x)=e^{2\pi i x}$. Then for every fixed $k$ there is a finite constant $C_k>0$ such that
\begin{equation}\label{eq:lemmaK-weil}
  |A_k(n,c)| \le C_k\,d(c)\,c^{1/2}
\end{equation}
uniformly in $n$ and $c$. Consequently, if $R_k(n)$ denotes the $c\ge2$ tail and $M_k(n)$ the $c=1$ term in the Rademacher series, then for every $\varepsilon>0$,
\begin{equation}\label{eq:lemmaK-tail}
  \frac{R_k(n)}{M_k(n)}=O_{k,\varepsilon}\!\left(n^{\varepsilon}e^{-c_k\sqrt n/2}\right).
\end{equation}
\end{lemma}

\begin{proof}[Proof of Lemma~K]
The phase $\nu_{\eta}(\gamma)^{-k}$ has unit modulus and finite conductor dividing~$24$; equivalently, after clearing the shift by $k/24$, the coefficients $A_k(n,c)$ are the standard level-$24$ Kloosterman sums twisted by a fixed finite-order multiplier. For such sums one has the usual square-root cancellation estimate
\[
  |A_k(n,c)| \ll_k d(c)\,(24n-k,k,c)^{1/2}c^{1/2},
\]
by the Weil bound in the presence of a fixed multiplier system; see for example~\cite{Weil1948,Iwaniec1997,goldfeld1983}. Here the gcd factor is bounded solely in terms of the fixed polar order $k$ and the conductor~$24$, so it is absorbed into the constant $C_k$, giving~\eqref{eq:lemmaK-weil}. The tail statement~\eqref{eq:lemmaK-tail} follows by inserting~\eqref{eq:lemmaK-weil} into the Rademacher series and applying the standard bounds $I_{\nu}(x)\ll_{\nu} e^x/\sqrt{x}$ for $x\ge1$ and $I_{\nu}(x)\ll_{\nu} x^{\nu}$ for $0<x\le1$; the detailed estimate is written out in the five-step proof below.
\end{proof}

\begin{proof}
We proceed in five standard steps.

\smallskip
\noindent\emph{1. Exact Rademacher expansion.}
Write $\lambda_n = n-k/24$, $\nu = 1 + k/2$, and $x_{n,c}=c_k\sqrt{\lambda_n}/c$ with $c_k=\pi\sqrt{2k/3}$. The Rademacher--Zuckerman expansion for the negative-weight modular form $\eta(\tau)^{-k}$ gives
\[
  p_k(n)
  = \mathcal{C}_k\,\lambda_n^{-(k+2)/4}
    \sum_{c\ge1} \frac{A_k(n,c)}{c}
    I_{\nu}(x_{n,c}),
\]
where $\mathcal{C}_k\neq 0$ depends only on $k$ and $A_k(n,1)=1$.

\smallskip
\noindent\emph{2. Identification of the conductor-$24$ sums.}
The coefficients $A_k(n,c)$ are exactly the generalized Kloosterman sums attached to the multiplier $\nu_{\eta}^{-k}$ and to the polar term $q^{-k/24}$. The conductor is $24$ because $\nu_{\eta}^{24}=1$, so only the residue class of $k\pmod{24}$ enters the local phases.
...
\[
  |A_k(n,c)| \le C_k\,d(c)\,c^{1/2}.
\]
Thus each summand is bounded by the classical square-root majorant for Kloosterman sums with a fixed eta-type multiplier.

\smallskip
\noindent\emph{4. Summation of the tail.}
Split the $c\ge2$ contribution into $2\le c\le \sqrt{\lambda_n}$ and $c>\sqrt{\lambda_n}$. Using
\[
  I_{\nu}(x)\ll_{\nu} \frac{e^x}{\sqrt{x}} \quad (x\ge1),
  \qquad
  I_{\nu}(x)\ll_{\nu} x^{\nu} \quad (0<x\le1),
\]
and the divisor bound $d(c)\ll_{\varepsilon} c^{\varepsilon}$, we obtain
\[
\begin{aligned}
  \sum_{c\ge2}\frac{|A_k(n,c)|}{c}I_{\nu}(x_{n,c})
  &\ll_k \sum_{2\le c\le \sqrt{\lambda_n}} d(c)c^{-1/2}\frac{e^{x_{n,c}}}{\sqrt{x_{n,c}}}
   + \sum_{c>\sqrt{\lambda_n}} d(c)c^{-1/2}x_{n,c}^{\nu} \\
  &\ll_{k,\varepsilon} n^{\varepsilon}e^{c_k\sqrt n/2}.
\end{aligned}
\]
After restoring the prefactor $\lambda_n^{-(k+2)/4}$, the entire $c\ge2$ tail is exponentially smaller than the $c=1$ term; more precisely,
\[
  \frac{R_k(n)}{\lambda_n^{-(k+2)/4}I_{\nu}(c_k\sqrt{\lambda_n})}
  = O_{k,\varepsilon}\!\left(n^{\varepsilon}e^{-c_k\sqrt n/2}\right).
\]
Hence the tail cannot affect the algebraic coefficients in the asymptotic expansion.

\smallskip
\noindent\emph{5. Extraction of $A_1^{(k)}$.}
The $c=1$ term therefore controls the full asymptotic expansion up to exponentially small relative error:
\[
  p_k(n)
  = \widetilde C_k\,\lambda_n^{-(k+2)/4} I_{1+k/2}(c_k\sqrt{\lambda_n})
    \left(1+O_{k,\varepsilon}\!\left(n^{\varepsilon}e^{-c_k\sqrt n/2}\right)\right).
\]
...
\[
  ...
  \right).
\]
Hence the coefficient of $n^{-1/2}$ is exactly
\[
...
\begin{table}[t]
\centering
\caption{Empirical Lemma~K constants from \texttt{g01\_lemma\_k\_eta\_report.json}, where $C_k^{\mathrm{emp}} = \max_{q\le 500} |A_k(1,q)|/(d(q)\,q^{1/2})$.}
\label{tab:lemmaK}
\begin{tabular}{rcc}
\toprule
$k$ & $C_k^{\mathrm{emp}}$ & $q_{\max}$ \\
\midrule
1  & 1.0766 & 89  \\
2  & 1.1607 & 443 \\
3  & 1.3042 & 181 \\
