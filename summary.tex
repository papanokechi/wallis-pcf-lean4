...
Independently, Lemma~\ref{lem:shift} and Step~3 in the proof of
Theorem~\ref{thm:wallis} give $S = S^{(1)} = 2S^{(0)} = 4/\pi$.
Hence $g_n/h_n \to \pi/4$, and passing~\eqref{eq:telescope} to the
limit yields the identity~\eqref{eq:seriesMain}.
